...
namespace Pic.Plugin.GeneratorCtrl
{
    ...
    public partial class GeneratorCtrl : UserControl, IGeneratorCtrl
    {
        ...
        private ILog _log = LogManager.GetLogger(typeof(GeneratorCtrl));
        #endregion
        ...
        public GeneratorCtrl()
        {
            ...
            LocalPluginDirectory = Path.GetTempPath();
        ...
        {
            LocalPluginDirectory = strComponentDirectory;
        }
        /// <summary>
        ...
        public void SetComponentFilePath(string pluginPath)
        {
            ...
            using (Tools pluginTools = new Tools(pluginPath, ComponentSearchMethod))
            {
                // guid
        ...
        public string PluginVersion { get; set; } = "2.0.0.0";
        #endregion
        ...
        public IComponentSearchMethod ComponentSearchMethod
        {
            ...
            get
            {
                ...
                {
                    string libraryPath = Path.Combine(LocalPluginDirectory, "Library");
        ...
        public string OutputPath
        {
            ...
            get
            {
                if (!_outputPathInitialized || string.IsNullOrEmpty(_outputPath))
                    _outputPath = Path.Combine(LocalPluginDirectory, txtName.Text.Replace(' ', '_') + ".dll");
                return _outputPath;
            }
        }

        public string LocalPluginDirectory { get; set; }
        #endregion
        ...
        private void OnSelectedIndexChangedErrors(object sender, EventArgs e)
        {
            try
            {
        ...
        private void OnGenerate(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    PluginGenerated?.Invoke(this, new GeneratorCtrlEventArgs(res.PathToAssembly));
                    using (PluginViewer frmPluginViewer = new PluginViewer(_componentSearchMethod, res.PathToAssembly))
                    {
                        if (frmPluginViewer.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            catch (IOException /*ex*/)
                            {
                                ...
                                {
                                    PluginViewerClosed?.Invoke(this, new GeneratorCtrlEventArgs(res.PathToAssembly));
                                    return;
                                }
                            }
                            // emit event if an event handler was defined
                            PluginValidated?.Invoke(this, new GeneratorCtrlEventArgs(OutputPath));
                        }
                        else
                        {
                            PluginViewerClosed?.Invoke(this, new GeneratorCtrlEventArgs(res.PathToAssembly));
        ...
        private void OnCheckedChangedThumbnail(object sender, EventArgs e)
        {
            fileSelectCtrl.Enabled = chbThumbnail.Checked;
        }
        private void OnBnClickNewGuid(object sender, EventArgs e)
        {
            txtGuid.Text = Guid.NewGuid().ToString();
        }
        private void OnBnClickComponentRef(object sender, EventArgs e)
        {
            try
            {
                string libraryPath = Path.Combine(LocalPluginDirectory, "Library");
                ...
                if (DialogResult.OK == form.ShowDialog())
                {
                    ...
                    {
                        SearchMethod = new ComponentSearchDirectory(libraryPath)
                    };
                    Component component = loader.LoadComponent(form._guid);

                    Tools tools = new Tools(component, new ComponentSearchDirectory(libraryPath));
