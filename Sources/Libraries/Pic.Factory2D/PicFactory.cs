...
namespace Pic
{
    namespace Factory2D
    {
        ...
        public class PicFactory : IDisposable, IEntityContainer, IEnumerable<PicEntity>
        {
            ...
            public PicFactory(List<PicTypedDrawable> entities)
            {
                AddEntities(entities, Transform2D.Identity);
            }
            public void Dispose()
            {
                _entities.Clear();
            ...
            public PicCardboardFormat InsertCardboardFormat(Vector2D position, Vector2D dimensions)
            {
                _cardboardFormat = new PicCardboardFormat(0, position, dimensions);
                return _cardboardFormat;
            }
            ...
            public void AddEntities(List<PicTypedDrawable> entities, Transform2D transform)
            {
                foreach (PicEntity entityIn in entities)
                {
