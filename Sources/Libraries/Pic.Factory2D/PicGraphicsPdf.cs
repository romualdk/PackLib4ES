...
namespace Pic.Factory2D
{
    public class PicGraphicsPdf : PicGraphics
    {
        ...
        public override void Initialize()
        {
            ...
            // get graphics
            pdfGfx = XGraphics.FromPdfPage(page);
            // draw a bounding box
            XRect rect = new XRect(0.5, 0.5, page.Width - 1, page.Height - 1);
            pdfGfx.DrawRectangle(XBrushes.White, rect);
            // initialize cotation
            PicCotation.GlobalCotationProperties.ArrowLength = XUnit.FromMillimeter(bbox.Height) / 50.0;
        }
        public override void DrawPoint(LT lineType, Vector2D pt)
        {
        }
        public override void DrawLine(LT lineType, Vector2D ptBeg, Vector2D ptEnd)
        {
            pdfGfx.DrawLine(new XPen(LineTypeToPdfColor(lineType))
                , DX(ptBeg.X), DY(ptBeg.Y)
                , DX(ptEnd.X), DY(ptEnd.Y));
        }
        public override void DrawArc(LT lineType, Vector2D ptCenter, double radius, double angleBeg, double angleEnd)
        {
            pdfGfx.DrawArc(new XPen(LineTypeToPdfColor(lineType))
        ...
        {
            XSize size = pdfGfx.MeasureString(text, ToFont(font, emSize));
        ...
        #region Helpers
        private double DX(double x) => XUnit.FromMillimeter((x - bbox.XMin));
        private double DY(double y) => XUnit.FromMillimeter((0.5 * bbox.Height + 0.5 * (bbox.YMin + bbox.YMax) - y));
        private double DL(double length) => XUnit.FromMillimeter(length);
        private XColor LineTypeToPdfColor(LT lType)
        {
            switch (lType)
            {
                case LT.LT_CUT: return XColors.Red;
                case LT.LT_PERFOCREASING: return XColors.Blue;
                case LT.LT_CONSTRUCTION: return XColors.Black;
                case LT.LT_PERFO: return XColors.Red;
                case LT.LT_HALFCUT: return XColors.LightBlue;
                case LT.LT_CREASING: return XColors.Blue;
                case LT.LT_AXIS: return XColors.Pink;
                case LT.LT_COTATION: return XColors.Green;
                default: return XColors.White;
            }
        }
        private XBrush LineTypeToPdfBrush(PicGraphics.LT lType)
        {
            switch (lType)
            {
                case LT.LT_CUT: return XBrushes.Red;
                case LT.LT_PERFOCREASING: return XBrushes.Blue;
                case LT.LT_CONSTRUCTION: return XBrushes.Black;
                case LT.LT_PERFO: return XBrushes.Red;
                case LT.LT_HALFCUT: return XBrushes.LightBlue;
                case LT.LT_CREASING: return XBrushes.Blue;
                case LT.LT_AXIS: return XBrushes.Pink;
                case LT.LT_COTATION: return XBrushes.Green;
                default: return XBrushes.White;
            }
        }
        private XFont ToFont(TextType tType, float emSize)
        {
            double fontHeight = pdfDocument.Pages[0].Height / 50.0;
            switch (tType)
            {
                case TextType.FT_COTATION: return new XFont("Arial", fontHeight);
                case TextType.FT_STRING: return new XFont("Arial", FontSizePt(emSize));
                default: throw new Exception("Unknown text type");
            }
        }
        public override float FontSizePt(float unitSize) => (float)XUnit.FromMillimeter((0.5 * bbox.Height + 0.5 * (bbox.YMin + bbox.YMax) - unitSize));
        public override Vector2D OffsetFont(float unitSize, VAlignment vAlignment) => Vector2D.Zero;
        #endregion
