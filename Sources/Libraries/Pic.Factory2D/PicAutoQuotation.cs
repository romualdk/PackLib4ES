...
namespace Pic.Factory2D
{
    ...
    {
        #region Data members
        private readonly double _tolerance = 1.0;
        #endregion
        ...
        public static void BuildQuotation(PicFactory factory)
        {
            ...
            for (short grp = 0; grp < 16; ++grp)
            {
                ...
                factory.ProcessVisitor(visitor
                    , new PicFilterAnd(new PicFilterGroup(grp), new PicFilterNot(new PicFilterLineType(PicGraphics.LT.LT_COTATION)))
                    );
        ...
        private void BuildHQuotation(PicFactory factory, short grp, Box2D box)
        {
            ...
            {
                if (typedDrawable.LineType == PicGraphics.LT.LT_CUT
                ...
                if (typedDrawable is PicSegment seg
                    && seg.IsVertical
                    && HasFoldLineType(seg))
                    vSegments.Add(seg);
            }
        ...
        private void BuildVQuotation(PicFactory factory, short grp, Box2D box)
        {
            ...
            foreach (PicTypedDrawable typedDrawable in list)
            {
                ...
                bbox.Extend(entityBox);
                // save vertical fold segments
                if (typedDrawable is PicSegment seg && seg.IsHorizontal)
                        hSegments.Add(seg);
            }
    ...
    internal class PicVisitorAutoQuotationEntities : PicFactoryVisitor
    {
        ...
        public override void Initialize(PicFactory factory) { Entities.Clear(); }
        public override void Finish() {}
        #endregion

        #region Public properties
        public List<PicTypedDrawable> Entities { get; } = new List<PicTypedDrawable>();
        #endregion

        #region Data members
        private short Grp { get; set; } = 0;
        private PicGraphics.LT[] ExcludedLineTypes = new PicGraphics.LT[]
        {
            PicGraphics.LT.LT_CONSTRUCTION,
            PicGraphics.LT.LT_COTATION,
            PicGraphics.LT.LT_AXIS,
            PicGraphics.LT.LT_ORIGIN,
            PicGraphics.LT.LT_BRIDGES 
        };
        #endregion
    }
    #endregion
