...
namespace PicParam
{
    ...
    public class LocalizerImpl : Pic.Plugin.ViewCtrl.ILocalizer, IDisposable
    {
            ...
            get
            {
                ...
                if (string.IsNullOrEmpty(_localizationFilePath))
                {
                    ...
                    _localizationFilePath = Path.Combine(
                        assemblyFolder
                        , $"Localisation_{CultureInfo.CurrentCulture.ThreeLetterWindowsLanguageName}.txt"
                        );
                }
        ...
        public void Load()
        {
            try
            {
                ...
                using (StreamReader readFile = new StreamReader(filePath, Encoding.Unicode))
                {
                    ...
                    while ((line = readFile.ReadLine()) != null)
                    {
                        ...
                        {
                            _log.Error($"Translation string dictionnary -> Skipping line: {line} with message: {ex.Message}");
                        }
                    }
                }
            }
            ...
            {
                _log.Error($"Failed loading translation strings from  file {LocalisationFileName} with message: {ex.Message}");
            }
        }
        /// <summary>
        ...
        public void Save()
        {
            ...
            {
                _log.Error($"Failed saving translation string as file {LocalisationFileName} with message : {ex.Message}");
            }
        }
        #endregion
