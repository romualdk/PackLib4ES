namespace Pic.DAL.SQLite
{
    ...
    public partial class PPDataContext
    {
        ...
        public PPDataContext()
            : base(new SQLiteConnection("DbLinqProvider=Sqlite;Data Source=" + ApplicationConfiguration.CustomSection.DatabasePath + ";FailIfMissing=false;",
                ApplicationConfiguration.CustomSection.DatabasePath.StartsWith("\\\\")))
        {
            RepositoryPath = ApplicationConfiguration.CustomSection.RepositoryPath;
            OnCreated();
        }
        public PPDataContext(string databaseFile, string repositoryPath)
            : base(new SQLiteConnection("DbLinqProvider=Sqlite;Data Source=" + databaseFile + ";FailIfMissing=false;"))
        {
            RepositoryPath = repositoryPath;
            OnCreated();
        }
        internal PPDataContext(DBDescriptor dbDesc)
            : base(new SQLiteConnection("DbLinqProvider=Sqlite;Data Source=" + dbDesc.DBFilePath + ";FailIfMissing=false;"))
        {
            RepositoryPath = dbDesc.RepositoryPath;
        }
        #endregion
    ...
    public partial class Component
    {
        ...
        {
            // retrieve component from ID
            Component comp = GetByGuid(db, guid);
            // find parameters
            ...
            foreach (ParamDefaultValue pdf in comp.ParamDefaultValues)
                defaultParamValues[pdf.Name] = pdf.Value;
            ...
            foreach (string name in dictNameValues.Keys)
            {
                ...
                if (!found) // if parameter was not found in the database, create it
                {
                    ParamDefaultValue pdf = new ParamDefaultValue
                    {
                        ComponentID = ID,
                        Name = name,
                        Value = (float)dictNameValues[name]
                    };
                    db.ParamDefaultValues.InsertOnSubmit(pdf);
                }
            }
        ...
        public void Delete(PPDataContext db, bool deleteDocument)
        {
            ...
            db.Components.DeleteOnSubmit(db.Components.Single(cp => cp.ID == ID));
            db.SubmitChanges();
            // delete document
            if (deleteDocument)
                Document.Delete(db);
        ...
        public override string ToString()
        {
            ...
            foreach (MajorationSet majSet in MajorationSets)
            {
