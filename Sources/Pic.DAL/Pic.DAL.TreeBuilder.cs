...
namespace Pic.DAL.SQLite
{
    ...
    public class TreeBuilder
    {
        ...
        public void BuildTree(TreeInterface treeImplementation)
        {
            ...
            // check database file
            if (!System.IO.File.Exists(ApplicationConfiguration.CustomSection.DatabasePath))
                _log.Error(string.Format("Failed to load database with path {0}", ApplicationConfiguration.CustomSection.DatabasePath));
            else
                _log.Info(string.Format("Database found:{0}", ApplicationConfiguration.CustomSection.DatabasePath)); 
            ...
            {
                var parentNodeObject = treeImplementation.InsertTreeNode(null, node);
